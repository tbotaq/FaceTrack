...
void *thread_facedetect( void *_arg_f )
{
  ...
  tmch -> calcMatchResult( human -> getResult(), arg_f -> templateImg, ci -> getImageSize(), arg_f -> center );
 ...
 INITIALIZATION://LABEL
  ...
  while( !hasBeenInitialized )
      ...
      while( faces<1 )
	{
      ...
      arg.templateImg = cvCreateImage( cvSize( arg.radius * 2, arg.radius * 2 ), IPL_DEPTH_8U, 1 );
      trackResult = -1;
      do
	{
  ...
  while( 1 )
    {	 
      ...
      cout<<"Getting human region"<<endl;
      do
	{
	  trackResult = human -> track();
	}while( trackResult == -1 );
	...
	abnormTimes = 0;
      ...
      cvShowImage( "Current Template Image", arg.templateImg );
