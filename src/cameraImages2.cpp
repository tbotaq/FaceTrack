...
int cameraImages::initialize()
{
  ...
  if( !frame )
    {
      printf("ERROR: Capturing error\n");
      exit(-1);
    }

  cvDepth = cvCreateImage(cvSize(frame->width, frame->height), IPL_DEPTH_16U, 1);
  cvInt = cvCreateImage(cvSize(frame->width, frame->height), IPL_DEPTH_8U, 1);
  cvConf = cvCreateImage(cvSize(frame->width, frame->height), IPL_DEPTH_16U, 1);
